...
public class AiChatModelGatewayService {
	...
	private AiProviderProperties.Provider resolveProvider(String providerCode) {
		// 统一交给 AiProviderService：优先读数据库 ai_provider（并解密其 API Key），
		// 数据库没有该 provider 时回退到 application.yml 的 app.ai.providers，完全向后兼容。
		return aiProviderService.resolve(providerCode);
	}
