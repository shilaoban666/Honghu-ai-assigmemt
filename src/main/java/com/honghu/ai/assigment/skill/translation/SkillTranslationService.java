package com.honghu.ai.assigment.skill.translation;
...
import com.honghu.ai.assigment.service.AiProviderService;
import com.honghu.ai.assigment.service.OpenAiCompatibleChatClient;
...
public class SkillTranslationService {
    ...
    /** 解析模型所属 provider 配置（DB 优先 + 解密 API Key，yaml 回退）。 */
    ...
    @Value("${app.skill.crawler.translate.model:deepseek-v4}")
    ...
    public List<String> translateToChinese(List<String> texts) {
        ...
        try {
            ...
            // 统一走 AiProviderService：优先读数据库 ai_provider（含后台加密保存的 API Key），
            // 数据库没有该 provider 时回退 application.yml；缺失/禁用会抛异常并被下方 catch 兜底回退英文。
            AiProviderProperties.Provider provider = aiProviderService.resolve(model.getProviderCode());
