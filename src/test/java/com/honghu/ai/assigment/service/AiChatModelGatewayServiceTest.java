...
class AiChatModelGatewayServiceTest {
    ...
    void setUp() {
        ...
        // 网关现在通过 AiProviderService 解析 provider；这里让 mock 复刻其“回退到 yaml 配置”的真实行为。
        lenient().when(aiProviderService.resolve(anyString())).thenAnswer(invocation -> {
            String code = invocation.getArgument(0);
            AiProviderProperties.Provider provider = aiProviderProperties.getProviders().get(code);
            if (provider == null) {
                throw new IllegalStateException("未找到 provider 配置: " + code);
            }
            return provider;
        });
